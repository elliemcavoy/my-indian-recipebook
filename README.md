...
<li>I would also like to improve the 'edit_recipe' functionality to make it easier for the user to edit their recipes. When they click on the 'edit recipe' button they are currently directed to the edit form which is a blank form. I would like the form to already contain all of the details of the recipe that they are trying to edit so that the user does not have to re-enter all of the details. Currently it will take the user a long time to re-type all of the details especially if they only need to edit one details, for example, a quantity for one of the ingredients.</li>
</ol>
...
<li>Card class taken from Materialize which includes a floating button.</li>
